...
🔵 Planned — **needs reconciliation with dada.stream before execution.**

This plan predates `~/projects/dada.stream`, which is now the authoritative architecture and contracts hub for the ecosystem. Sections below that propose inventing manifests, event envelopes, `RoutableContent`, capability declarations, and entity schema bases are **superseded** by dada.stream's existing specs:

- Service manifest → `~/projects/dada.stream/platform/contracts/service-manifest.md`
- Event envelope + taxonomy → `~/projects/dada.stream/platform/contracts/event-types.md`
- API conventions → `~/projects/dada.stream/platform/contracts/api-conventions.md`
- Content envelope (replaces `RoutableContent`) → `~/projects/dada.stream/platform/domains/content-model.md`
- Topic model → `~/projects/dada.stream/platform/domains/topic-model.md`
- Layered system view → `~/projects/dada.stream/platform/architecture/system-overview.md`

When picked up, the work is mostly **conformance to dada.stream's existing specs**, not inventing new ones. The migration vehicle also brings this repo into `dada.stream/components/x-sync/` (see the classifier plan at `~/projects/dada.stream/components/classifier/.project/plans/2026-04-18-classification-service-foundation.md` for the target topology and for the interim envelope-mapper pattern that lets consumers work against us before conformance lands).

Read the rest of this plan through that lens: the ecosystem-framing sections are still correct; the proposed *contract shapes* are out of date.
